...
export default function AnimatedHomepage() {
  ...
  // 根据鼠标x位置决定音符
  const getNoteFromPosition = useCallback((x: number, canvasWidth: number) => {
    const numNotes = NOTES.length;
    // 确定当前x落在哪一个音符区间
    const noteIndex = Math.floor((x / canvasWidth) * numNotes);
    const clampedIndex = Math.max(0, Math.min(noteIndex, numNotes - 1));
  ...
  const debouncedInteractionHandler = useMemo(
    () =>
      debounce((event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
        ...
        if (currentTime - lastSoundTimeRef.current > 100) {
          // 根据X坐标选择音符
          const frequency = getNoteFromPosition(x, canvas.width);
          playSound(frequency);
    ...
    [createShape, playSound, getNoteFromPosition]
  );
